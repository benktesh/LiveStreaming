...
public class HttpServer extends AbstractVerticle {
	...
	public void start() throws Exception {
		...
		router.get("/stream/:id/:filename").handler(this::stream);
		router.get("/create/:filename").handler(this::createstream);
		router.delete("/delete/:id").handler(this::deletestream);
	...
	private void index(RoutingContext routingContext) {
		...
		routingContext.request().response().setStatusCode(HttpStatus.SC_OK).end(indexFile);

	}

	private void deletestream(RoutingContext routingContext) {
		String methodName = new Object() {
		}.getClass().getEnclosingMethod().getName();
		logMethodCall(Arrays.asList("Called:", methodName));
		routingContext.request().response().setStatusCode(HttpStatus.SC_OK).end(routingContext.request().path());
	}
	...
	private void createstream(RoutingContext routingContext) {
		...
		JsonObject streamingData = new JsonObject(Json.encode(basicConfig));

		String sourceFile = Paths.get(basicConfig.sourceFolder, filename).toString();
		streamingData.put("sourceFile", sourceFile);
	...
	private void stream(RoutingContext routingContext) {
		String methodName = new Object() {
		}.getClass().getEnclosingMethod().getName();
		logMethodCall(Arrays.asList("Called:", methodName));
		...
		String pilePath = Paths.get(basicConfig.streamingRoot, fileId, "StreamingHLS", filename).toString();
		if (filename.equalsIgnoreCase("manifest")) {
			pilePath = Paths.get(basicConfig.streamingRoot, fileId, "StreamingSmooth", filename).toString();
		}

		sendAFile(routingContext.request().response(), pilePath);
		// routingContext.request().response().setStatusCode(HttpStatus.SC_OK).end(routingContext.request().path());
